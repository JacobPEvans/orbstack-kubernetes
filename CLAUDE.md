...
- **Overlays are gitignored**: `k8s/monitoring/overlays/local/` is generated at deploy time by `scripts/generate-overlay.sh` and must not be committed.
...
1. `make deploy-doppler` (or `kubectl apply -k k8s/monitoring/overlays/local/` if SOPS key unavailable)
...
- `k8s/monitoring/overlays/local/` - Generated overlay with real volume paths (gitignored)
